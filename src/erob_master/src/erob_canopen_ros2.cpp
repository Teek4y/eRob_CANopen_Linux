...
public:
    ...
    CANopenROS2() : Node("simple_erob_control")
    {
        ...
        motor_config_[2].rated_torque = 31000;
        motor_config_[3].rated_torque = 31000;
        motor_config_[4].rated_torque = 31000;
...
private:
    ...
    void initialize_node(MotorConfig motor_config)
    {
        ...
        // 验证操作模式
        // int32_t mode = 
        read_sdo(node_id, OD_OPERATION_MODE_DISPLAY, 0x00);
    ...
    void configure_pdo(int node_id)
    {
        ...
        // 配置TxPDO2
        {
        // RCLCPP_INFO(this->get_logger(), "开始配置TxPDO2");
        
        // 1. 禁用TxPDO2
        uint32_t txpdo2_cob_id = COB_TPDO2 + node_id;
        write_sdo(node_id, 0x1801, 0x01, txpdo2_cob_id | 0x80000000, 4);
        std::this_thread::sleep_for(std::chrono::milliseconds(10));
        
        // 2. 设置传输类型
        write_sdo(node_id, 0x1801, 0x02, 0x01, 1);
        std::this_thread::sleep_for(std::chrono::milliseconds(10));
        ...
        // 配置RxPDO3 用于PTM/CST
        {
        // RCLCPP_INFO(this->get_logger(), "开始配置RxPDO3");
        ...
        switch(cob_id){
            case COB_TSDO:{
                ...
                else if (index == OD_STATUS_WORD && subindex == 0x00)  // 状态字
                {
                    ...
                    {
                        RCLCPP_WARN(this->get_logger(), "电机处于故障状态");
                        motor_config_[node_id-1].status_fault = 1;
                    }
                }
    ...
    // 各类辅助函数
{
    ...
    // 辅助函数：位置脉冲转速度
    float pulse_to_velocity(int32_t pulse)
    {
        float velocity = (static_cast<float>(pulse) / ENCODER_RESOLUTION) * 360.0;
        return velocity;
    }
    ...
    void set_velocity(int node_id, float velocity_deg_per_sec)
    {
        ...
        if (mode != MODE_PROFILE_VELOCITY && mode != MODE_PROFILE_VELOCITY)
        {
